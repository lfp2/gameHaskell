...
data GameState = Level Int | GameOver
...
main =
  ...
  in funInit winConfig gameMap [bar1,bar2,ball] Level 1 gameAttribute bindings gameCycle Idle []
...
gameCycle = do
  ...
  case gState of
        Level n -> case n of
          1 -> do
                if(n1 == 3 || n2 == 3)
                  then (do setGameState(Level n+1))
                  else (do
